...
public class Rect {
    ...
    public boolean contains(Coordinate p) {
        return (((width > 0) && ((left <= p.x) && (right >= p.x))) ||
        ((width < 0) && ((left >= p.x) && (right <= p.x)))) &&
        (((height > 0) && ((top <= p.y) && (bottom >= p.y))) ||
        ((height < 0) && ((top >= p.y) && (bottom <= p.y))));
    }

    private boolean crosses(double l1x, double l1y1, double l1y2, double l2y,
        double l2x1, double l2x2) {
    ...
    public boolean overlaps(Rect r) {
        ...
        if (crosses(r.left, r.top, r.bottom, top, left, right)) {
            return true;
        }

        if (crosses(r.left, r.top, r.bottom, bottom, left, right)) {
            return true;
        }

        if (crosses(r.right, r.top, r.bottom, top, left, right)) {
            return true;
        }

        if (crosses(r.right, r.top, r.bottom, bottom, left, right)) {
            return true;
        }

        if (crosses(r.top, r.left, r.right, left, top, bottom)) {
            return true;
        }
        ...
        if (crosses(r.bottom, r.left, r.right, top, left, right)) {
            return true;
        }

        if (crosses(r.bottom, r.left, r.right, right, top, bottom)) {
            return true;
        }
    ...
    public Rect convert(Rect r) {
        return new Rect(convertX(r.left), convertY(r.top), convertX(r.right),
            convertY(r.bottom));
    }
    ...
    public Rect revConvert(Rect r) {
        return new Rect(revConvertX(r.left), revConvertY(r.top),
