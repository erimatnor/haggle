...
public class Haggle extends MonitoredNetwork {
	...
	public synchronized SensorNode addNode(String name) {
		...
		if (retval == null) {
			retval = new SensorNode(nextNodeID, name);
	 ...
	 * 
	 * @see vendetta.Overlay#overlayStarted(vendetta.MonitorNode,
	 * java.lang.String)
	 */
	...
	public int countNodes(String filename) {
		 ...
		 * (s.hasNextLine()) { if (!s.nextLine().startsWith("#")) { n++; } } }
		 * catch (IOException ioe) { LOG.error("Failed to count nodes: ", ioe);
		 * return -1; }
		 * 
		 * return n;
		 */
