...
public class scenariorunner implements Runnable {
	...
	public static void main_run(String[] args) {
		...
		/*
		 * int nodeCount = 0; { String tmp; tmp = getTagContent(scenario_file,
		 * "nodecount"); if(tmp != null) nodeCount = Integer.parseInt(tmp); }
		...
		for (i = 0; i < nodeCount; i++) {
			mySystem("start_program_on_node.sh" + " node-" + i + " haggle"
					+ " \"--non-interactive\"");
		}
		...
		if (appName != null) {
			for (i = 0; i < nodeCount; i++) {
				mySystem("stop_program_on_node.sh" + " node-" + i + " "
						+ appName);
			}
			...
			for (i = 0; i < nodeCount; i++) {
				mySystem("wait_for_app_to_stop.sh" + " node-" + i + " "
						+ appName);
			}
		}
		try {
		...
		// tarball the result:
		mySystem("save_logs.sh " + nodeCount + " "
				+ getFileName(scenario_file_name));
		saveLogs_ok = true;
		if (cancelButton_pressed)
			return;
