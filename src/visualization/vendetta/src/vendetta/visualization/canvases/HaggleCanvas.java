...
public class HaggleCanvas extends Vendetta3DCanvas implements MouseWheelListener {
    ...
    private class SensorWindow {
		...
		public void clear()
		{
			window_height = 0.0;
			subwindows = 
				new TreeMap<SensorNode,SensorWindow>(
    ...
    private void initFromConfigFile(String configFile) {
		...
		if(mode.equals("window array"))
			...
			throw new 
				RuntimeException(
					"Failed to read canvas configuration file, mode = " + 
					mode + ".");
    }
	...
	/*
	void drawLines(DrawingSurface ds)
	{
		Color	gray = new Color(0.75f,0.75f,0.75f);
		ds.drawLine(
			allBounds.topLeft(),
			node.getDisplayedPosition(),
			HaggleCanvas.default_line_width/2,
			gray);
		ds.drawLine(
			allBounds.topRight(),
			node.getDisplayedPosition(),
			...
			gray);
		ds.drawLine(
			allBounds.bottomLeft(),
			node.getDisplayedPosition(),
			HaggleCanvas.default_line_width/2,
			gray);
		ds.drawLine(
			allBounds.bottomRight(),
			node.getDisplayedPosition(),
			HaggleCanvas.default_line_width/2,
			gray);
	}
	
	void draw(DrawingSurface ds)
	{
		ds.fillRect(dragBounds, new Color(0,0,0));
		ds.fillRect(closeBounds, new Color(1.0f,0.75f,0.75f));
		ds.pushBounds(contentBounds);
		node.doRender(ds);
		clickRect = ds.popBounds();
	}
	*/
	...
	private void animateSensorNodes(SensorNode[] node)
	{
		...
		for(i = 0; i < node.length; i++)
		{
			node[i].setCurrentPosition(
				node[i].getCalculatedPosition());
			if(node[i].getDisplayedPosition().
			   distance(node[i].getCalculatedPosition()) > 0.001)
			{
				node[i].setDisplayedPosition(
					node[i].getCalculatedPosition());
			}
		}
	}
	...
	@SuppressWarnings({"unchecked"})
	private void renderConnectivityWindow(
					DrawingSurface ds, 
					SensorWindow win, 
					SensorNode[] node)
	{
		...
		ds.fillCircle(
			...
			0.9,
			0.001/connectivityScale,
			new Color(0.75f,0.75f,0.75f),
			Color.white);
		// Draw origin:
		ds.fillCircle(
			new Coordinate(0.5,0.5),
			0.002/connectivityScale,
			0.0,
			null,
			new Color(0.75f,0.75f,0.75f));
		
		Object[] entries = 
			win.subwindows.entrySet().toArray();
		
		// draw lines to subwindows:
		for(i = 0; i < entries.length; i++)
		{
			SensorWindow Win = 
				((Map.Entry<SensorNode,SensorWindow>) 
				(entries[i])).getValue();
			SensorNode Node = 
				((Map.Entry<SensorNode,SensorWindow>) 
				(entries[i])).getKey();
		...
		for(i = 0; i < link.length; i++)
		{
			...
			{
				ds.drawLine(
					squareToPos(n1.getDisplayedPosition()),
		...
		for(i = 0; i < node.length; i++)
		{
			node[i].setRect(
				ds.fillCircle(
					...
					node[i].getCurrentSize(),
					0.1,
					node[i].getOutlineColor(),
					node[i].getColor()));
			if(!node[i].isAlive())
				ds.fillCircle(
					squareToPos(node[i].getDisplayedPosition()),
					node[i].getCurrentSize()*0.4,
					0.0,
					null,
					Color.white);
		}
	...
	@SuppressWarnings({"unchecked"})
	private void renderWindowArrayWindow(
					DrawingSurface ds, 
					SensorWindow win, 
					SensorNode[] node)
	{
		...
		ds.fillRect(0, 0, 1, 1, Color.white);
		// Get ourselves a square-looking drawing area:
		ds.pushBounds(0,0,1,1.0/win.window_height);
		...
		if(need_to_recalculate_bounds)
		{
			...
			for(i = 0; i < entries.length; i++)
			{
				...
				Rect r = 
					new Rect(
						offx + max_size*x + 0.01,     offy + max_size*y + 0.01, 
						offx + max_size*(x+1) - 0.01, offy + max_size*(y+1) - 0.01);
				sub.setBounds(
					r,
					null,
					null);
			...
			for(j = i+1; j < entries.length; j++)
			{
					...
					subj.node.isConnectedTo(subi.node.getNodeID()))
				{
					...
					offset = 
						offset*0.99*
						(((double)(j-i))/
						 ((double)(entries.length-1-i)));
					
					iconn = 
						new Coordinate(
							subi.allBounds.right-offset,
	...
	private void renderInternalStateWindow(
					DrawingSurface ds, 
					SensorWindow win, 
					SensorNode[] node)
	{
	...
	private void render(DrawingSurface ds, SensorWindow win, SensorNode[] node) 
	{
			...
			if(win.dragBounds != null)
				if(win.closeBounds != null)
					ds.drawStringTopLeft(
						win.node.getNodeName(),
						win.closeBounds.topRight(),
						1.5*win.contentBounds.width,
						Color.white);
				else
					ds.drawStringTopLeft(
						win.node.getNodeName(),
		...
		switch(win.type)
		{
			...
			default:
				System.out.println(
					"Unknown window type in render(): " + win.type);
			break;
		}
		ds.popBounds();
	}
	...
	{
		SensorWindow[] subwindow = 
			win.subwindows.values().toArray(new SensorWindow[0]);
		int i;
	...
	private void mouseReleasedConnectivity(
					MouseEvent e, 
					Coordinate c, 
					SensorWindow win)
	{
		...
		c = 
			new Rect(
				p.x - win.window_height/2.0,
				p.y - win.window_height/2.0,
				p.x + win.window_height/2.0,
				p.y + win.window_height/2.0).revConvert(
			new Rect(
			(1.0 - connectivityScale)/2.0,
			(win.window_height - connectivityScale)/(2.0*win.window_height),
			(1.0 + connectivityScale)/2.0,
			...
			if(mouseButton == 1)
			{
				...
				switch(e.getClickCount())
				{
					case 1:
						...
						{
							for(i = 0; 
								i < Vendetta.getMonitorNodeCount(); 
								i++)
							{
								s = lh.getSensorNode(i);
								if(s != null)
	...
	private void mouseReleasedWindowArray(
					MouseEvent e, 
					Coordinate c, 
					SensorWindow win)
	{
	...
	private void mouseReleasedInternalState(
					MouseEvent e, 
					Coordinate c, 
					SensorWindow win)
	{
	...
	private void mouseReleased(MouseEvent e, Coordinate c, SensorWindow win)
	{
		...
		switch(win.type)
		{
			...
			default:
				System.out.println(
					"Unknown window type in mouseReleased(): " + win.type);
			break;
		}
	}
	...
	public synchronized boolean startDrag(
		Coordinate c, 
		int mouseButton, 
		SensorWindow win)
	{
	...
	public synchronized void dragTo(
		SensorNode s, 
		Coordinate c, 
		int mouseButton,
		SensorWindow win)
	...
	private void mouseDraggedConnectivity(
					MouseEvent e, 
					Coordinate c, 
					Coordinate mp, 
					SensorWindow win)
	...
	private void mouseDraggedWindowArray(
					MouseEvent e, 
					Coordinate c, 
					Coordinate mp, 
					SensorWindow win)
	...
	private void mouseDraggedInternalState(
					MouseEvent e, 
					Coordinate c, 
					Coordinate mp, 
					SensorWindow win)
					...
					SensorWindow win)
	{
		...
		switch(win.type)
		{
			...
			default:
				System.out.println(
					"Unknown window type in mouseDragged(): " + win.type);
			break;
		}
	}
    ...
    public synchronized void mouseWheelMoved(MouseWheelEvent e)
    {
		...
		connectivityScale =
			connectivityScale +
			((double)e.getUnitsToScroll()) * 0.01;
