		...
		while(true)
		{
			try {
				if(fromIP != null)
				{
					...
					should_wait = false;
					progressDebug(
						new Date() + ": Getting XML from " + ip);
					String xml = getXMLFromHaggle(ip);
					if(xml != null)
					{
						progressDebug(
							new Date() + ": Got XML from " + ip);
						xslDebug("XML: " + xml);
						...
						if(number_of_fails < 3)
						{
							...
							should_wait = true;
							progressDebug(
								new Date() + ": Didn't get XML from " + ip);
						}else{
							fromIP = null;
							should_wait = true;
							number_of_fails = 0;
...
public class SensorNode extends MonitorNode {
	private class Animation {
		...
		Animation(
			String fromDO, boolean fromNode,
			String throughDO, boolean throughNode,
			String toDO, boolean toNode,
			long length,
			Color col)
		{
			if(fromDO == null)
				throw new 
					RuntimeException("no \"from\" DO.");
			if(toDO == null)
				throw new 
					RuntimeException("no \"to\" DO.");
	...
	public boolean isConnectedTo(String nodeID)
	{
		...
		{
			DataObject dObj = 
				DataObjectTable.getDO(
					DataObjectTable.indexByNodeID(
						nodeID));
			if(dObj != null)
	...
	private void resolveHostname() {
		try {
			...
			throw new 
				RuntimeException(
					"Failed to resolve hostname: " + uhe.getMessage());
		}
	}
			...
			if(split.length < 3)
				...
				}else if("EVENT_TYPE_NODE_CONTACT_NEW".equals(split[2]))
				{
					if(split.length < 5)
						...
						DataObject dObj = 
							DataObjectTable.getDO(
								DataObjectTable.indexByNodeID(split[4]));
						if(dObj != null)
							...
							split[6] != null)
						{
								...
								!split[4].equals(node2.getID()))
							{
								animations.add(
									new Animation(
										split[4], false,
										node1.getNodeID(), true,
										node2.getNodeID(), true,
										60,
				...
				}else if("EVENT_TYPE_DATAOBJECT_SEND_SUCCESSFUL".equals(split[2]))
				{
					if(split.length < 6)
						LOG.debug("Bogus event: " + evt);
					else
				...
				}else if("LE_SEND_SECURITY_FAILURE".equals(split[2]))
				{
					if(split.length < 5)
						...
						DataObject dObj = 
							DataObjectTable.getDO(
								DataObjectTable.indexByID(split[4]));
						if(dObj != null)
	...
	private Coordinate asNodePos(Coordinate p)
	{
		return 
			new Coordinate(
				(p.x/2.0 + 0.5), 
				0.5 + spacing + (p.y/2.0 + 0.5)*(0.5 - spacing));
	}
	...
	private Coordinate revAsNodePos(Coordinate p)
	{
		return 
			new Coordinate(
				(p.x - 0.5)*2.0, 
				((p.y - (0.5 + spacing))/(0.5 - spacing) - 0.5)*2.0);
	}
	...
	public synchronized void doRender(DrawingSurface ds)
	{
		...
		// circle:
		ds.fillOval(
			0.0+line_width/2.0,0.5 + spacing+line_width/2.0,
			1.0-line_width/2.0,1.0-line_width/2.0,
		...
		// nodes)
		{
			...
			for(i = 0; i < entries.length; i++)
			{
				...
				int do1_index = 
					DataObjectTable.indexByNodeID(entry1.getKey());
		...
		// coordinate).
		{
			TreeMap<Double, DataObject> dObjMap = 
				new TreeMap<Double, DataObject>();
			...
			for(i = 0; i < DataObjectTable.getLength(); i++)
			{
				...
				{
					dObjMap.put(
						new Double(dObj.getDisplayedPosition().y), 
						dObj);
				}
			}
			...
			DataObject[] dObj = 
				(DataObject[]) (
					dObjMap.values().toArray(new DataObject[0]));
			if(dObj != null)
				for(i = 0; i < dObj.length; i++)
				{
					...
					ds.drawLine(
						...
						line_width/2,
						dObj[i].getNodeDOLinkColor());
					
					// Draw:
					dObj[i].setNodeRect(
						ds.fillCube(
							asNodePos(dObj[i].getDisplayedPosition()),
							dObj[i].getCurrentNodeSize(),
							0.05,
		...
		ds.fillOval(
			0.0+line_width/2.0,0.0+line_width/2.0,
		...
		{
			Object[] entry = 
				do_links.entrySet().toArray();
		
			for(i = 0; i < entry.length; i++)
			{
				int do1_index = 
					DataObjectTable.indexByRowID(
						((Map.Entry<String,TreeMap<String, Float>>)
						 entry[i]).getKey());
				Object[] Js = 
					((Map.Entry<String,TreeMap<String,Float>>)
					 entry[i]).getValue().entrySet().toArray();
				int j;
				for(j = 0; j < Js.length; j++)
				{
					Map.Entry<String,Float> entry2 = 
						...
						(Js[j]);
					int do2_index = 
						DataObjectTable.indexByRowID(entry2.getKey());
					if(do1_index < do2_index)
					{
						DataObject dObj1 = 
							DataObjectTable.getDO(do1_index);
						...
						if(dObj1 != null && dObj2 != null)
						{
							...
							{
								if(	show_do_do_links ||
									dObj1.isNodeDescription() ||
									dObj2.isNodeDescription())
								{
									ds.drawLine(
										asDOPos(
											dObj1.getCurrentPosition()),
										asDOPos(
											dObj2.getCurrentPosition()),
		...
		DataObject[] dObj = null;
		{
			...
			for(i = 0; i < DataObjectTable.getLength(); i++)
			{
				...
				dObjMap.put(
					new Double(dO.getDisplayedPosition().y), 
					dO);
			}
				...
				for(i = 0; i < dObj.length; i++)
					if(dObj[i].isVisible())
					{
						...
						dObj[i].setDORect(
							ds.fillCircle(
								asDOPos(dObj[i].getDisplayedPosition()),
								dObj[i].getCurrentSize(),
								0.1,
								dObj[i].getDOOutlineColor(),
								dObj[i].getDOColor()));
					}
				// Show hit rects?
				if(false)
				...
				for(i = 0; i < dObj.length; i++)
						...
						(dObj[i].isSelected() || dObj[i].isAnimating()))
					{
						...
						if(dObj[i].isSelected())
						{
							...
							Rect r = 
								new Rect(
									p.x - width/2 - 2*borderSize, 
									p.y - height - up - 2*borderSize,
									p.x + width/2 + 2*borderSize, 
									p.y - up + 2*borderSize);
						...
						if(dObj[i].getThumbnail() != null)
						{
							...
							ds.drawImageInRect(
								new Rect(
									p.x - 2*w,
									p.y - height - up - 2*borderSize - 
										4*s.height*w/s.width - up/2,
									p.x + 2*w,
									p.y - height - up - 2*borderSize - up/2), 
								dObj[i].getThumbnail());
						}
					}
			}
	...
	private Coordinate getPos(Animation.EndPoint ep)
	{
		if(ep != null)
		{
			...
			if(ep.nodeTarget)
				o =
					DataObjectTable.getDO(
						DataObjectTable.indexByNodeID(ep.DataObjectID));
			else
				o =
					DataObjectTable.getDO(
						DataObjectTable.indexByID(ep.DataObjectID));
			if(o != null)
				if(ep.nodeTarget)
	...
	public synchronized String getTagContent(
							Node node, 
							String element_name, 
							int num)
	{
					...
					for(i = 0; i < DOs.getLength(); i++)
					{
						...
						if(id != null && rowid != null)
						{
							...
							if(thisNode != null)
								...
								parseDebug(
									"Found DO: " + id + " rowid: " + rowid);
							DataObjectTable.ensure_do_is_in_table(id, rowid);
							DataObject dObj = 
								DataObjectTable.getDO(
									DataObjectTable.indexByRowID(rowid));
							if(dObj != null)
							{
								if(dObj.isSpecialDO())
									dObj.setNew();
								...
								if(dObj.isNodeDescription())
								{
									...
									for(j = 0; 
										j < DataObjectTable.getLength(); 
										j++)
									{
										DataObject dObj2 = 
											DataObjectTable.getDO(j);
										if(dObj2 != null)
											if(	dObj2 != dObj && 
												dObj2.isNodeDescription())
														...
														dObj.getNodeID()))
												{
													...
													dObj.setCurrentPosition(
														pos);
													dObj.setCalculatedPosition(
														pos);
													dObj.setDisplayedPosition(
														pos);
												}
									}
								}
			...
			
			{
				...
				if(maps != null)
				{
					...
					for(i = 0; i < maps.getLength(); i++)
					{
						...
						{
							parseDebug(
								"Found Node, node rowid: " + 
								node_rowid + " DO rowid: " + dataobject_rowid);
							DataObject dObj = 
								DataObjectTable.
								getDO(
									DataObjectTable.
			...
			
			{
				...
				if(relations != null)
				{
					...
					for(i = 0; i < relations.getLength(); i++)
					{
						...
						NodeList matching_attrs = 
							getSubTags(relations.item(i), "matching_attribute");

						int numMatches = matching_attrs.getLength();
						
						NodeList dObjs = 
							getSubTags(relations.item(i), "dataobject");
						...
						if(dObjs != null)
						{
							...
							for(j = 0; j < dObjs.getLength(); j++)
							{
								...
								if (str_do1_num_attrs != null) 
									do1_num_attrs = Integer.parseInt(str_do1_num_attrs);
								...
								if(do1_rowid != null)
									for(k = j+1; k < dObjs.getLength(); k++)
									{
										...
										if (str_do2_num_attrs != null) 
											do2_num_attrs = Integer.parseInt(str_do2_num_attrs);
										...
										if(do2_rowid != null)
										{
											parseDebug("Found DO-DO relation:" +
													   ...
													   do2_rowid);
											if (numMatches > 0) {
												addDOLink(do1_rowid, do2_rowid, 100*numMatches/do1_num_attrs);
/*												if (do1_num_attrs > do2_num_attrs) {
													if (do1_num_attrs > 0) {
														addDOLink(do1_rowid, do2_rowid, 100*numMatches/do1_num_attrs);
													}
												} else {
													if (do2_num_attrs > 0) {
														addDOLink(do2_rowid, do2_rowid, 100*numMatches/do2_num_attrs);
													}
												} */
											}
										}
				...
				if(neighbors != null)
				{
					for(i = 0; i < neighbors.getLength(); i++)
					{
						...
						if(node_id != null)
						{
							DataObject dObj =
								DataObjectTable.getDO(
									DataObjectTable.indexByNodeID(node_id));
							if(dObj != null)
							{
	...
	@SuppressWarnings({"unchecked"})
	public synchronized Coordinate getForceForPosition(
									Coordinate pos, 
									DataObject dObj)
	{
		int i;
		...
		// Give the edge of the circle a repelling force:
		{
			...
			if(!centerThisNode || !dObj.isThisNode())
			{
				...
				if(F > forceMinDistance)
				{
					...
					posProject.scale(-1.0/F);
					// Calculate the distance from the original point to the edge:
					F = 1.0 - F;
					// Scale the projected point to become a repelling force from 
					// the edge:
					posProject.scale(1.0/(F*F) - 1.0);
		...
		if(!centerThisNode || !dObj.isThisNode())
		{
						...
						if(F > forceMinDistance && F < forceMaxDistance)
						{
							...
							if(
								...
								!(otherObject.isNodeDescription() &&
								  dObj.isNodeDescription() &&
								  otherObject.getNodeID().equals(dObj.getNodeID())))
							{
								// Set to repel:
				...
				if(set != null)
				{
					...
					for(i = 0; i < arr.length; i++)
					{
						...
						if(otherObject != null)
						{
							...
							{
								if(
									// No attracting force between an object and itself:
									otherObject != dObj &&
									// No attracting forces between non-NDs unless those 
									// links are shown:
									(show_do_do_links ||
									 dObj.isNodeDescription() ||
									 otherObject.isNodeDescription()))
			...
			// Give those node descriptions that are linked to this one attracting 
			// forces:
			...
			for(i = 0; i < DataObjectTable.getLength(); i++)
			{
				...
				if(dObj != null)
				{
					if(!dObj.isBeingDragged())
					{
						...
						if(dObj.getDisplayedPosition().
						   distance(dObj.getCalculatedPosition()) > 0.001)
						{
							do_redraw = true;
						}
					}
	...
	public synchronized void handleClick(Coordinate c, int mouseButton, boolean shiftClick)
	{
		int i;
	...
	public synchronized void dragTo(DataObject dObj, Coordinate c, int mouseButton)
	{
		if(mouseButton != 1)
			return;
