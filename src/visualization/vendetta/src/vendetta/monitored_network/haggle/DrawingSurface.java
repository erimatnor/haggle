...
public class DrawingSurface {
	...
	public void drawStringTopLeft(String str, Coordinate p, double scale,
		...
		r = measureString(str, 0, 0, scale);
		p = new Coordinate(p.x - r.left, p.y - r.top);
		r = new Rect(p.x - r.left, p.y + r.top, p.x - r.left + r.width, p.y
				+ r.top + r.height);
		fillRect(r, bgCol);
		drawString(str, p, scale, textCol);
	}
	...
	public void drawStringCentered(String str, Coordinate p, double scale,
		...
		Rect r;
		r = measureString(str, 0, 0, scale);
		p = new Coordinate(p.x - (r.right - r.left) / 2.0, p.y
				- (r.bottom - r.top) / 2.0);
	...
	public void drawBentArrow(Coordinate p1, double head_size_1, Coordinate p2,
			double head_size_2, double width, Coordinate displace,
	...
	public Rect fillCube(Coordinate p, double size, double edgeWidth,
		...
		return fillCube(p.x - size / 2, p.y - size / 2, p.x + size / 2, p.y
				+ size / 2, edgeWidth, edgeColor, surfaceColor);
	}
