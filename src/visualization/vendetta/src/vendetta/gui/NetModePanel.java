...
public class NetModePanel extends JTabbedPane implements ActionListener {
	...
	private JPanel panel_Filter, panel_Node, panel_Commands, panel_Testbed;
	...
	private String cmd_Node[], cmd_Overlay[];
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == button_UpdateFilter) {
			...
			int i;
			vendetta.MonitorNode[] node = Vendetta.getSelectedMonitorNodes().clone();
			...
			for (int i = 0; i < buttons_Node.length; i++) {
				if (e.getSource() == buttons_Node[i]) {
					...
					if (args.equals(""))
						args = null;
					performCommand(command, args, !buttons_Node_activeWithNodeSelected[i], buttons_Node_onlyOneExec[i]);
			...
			for (int i = 0; i < buttons_Testbed.length; i++) {
				if (e.getSource() == buttons_Testbed[i]) {
					...
					if (args.equals(""))
						args = null;
					performCommand(command, args, !buttons_Testbed_activeWithNodeSelected[i], buttons_Testbed_onlyOneExec[i]);
	...
	private void performCommand(String[] command, String args, boolean withoutnodes, boolean onlyOneExec) {
