...
public class GUI extends JFrame implements ActionListener,
	...
	public void initGUI(Splash splash, MonitorNode[] nodes,
			String[] tableColNames, VSettings vsettings,
			Vendetta.PlaybackMode mode) {
		this.mode = mode;
	...
	private void initMenuBar() {
		...
		miShowForwardingDOs.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F9, 0));
...
//				.
		...
		if (e.getSource() == button_close || e.getSource() == miExit) {
			...
			for (int i = 0; i < Vendetta.getMonitorNodeCount(); i++) {
				if(Vendetta.getMonitorNode(i) != null)
					Vendetta.getMonitorNode(i).
						setShowDODOLinks(
							miShowLinks.getState());
			}
		} else if (e.getSource() == miShowForwardingDOs) {
			for (int i = 0; i < Vendetta.getMonitorNodeCount(); i++) {
				if(Vendetta.getMonitorNode(i) != null)
					Vendetta.getMonitorNode(i).
						setHideForwardingDOs(
	...
	private void calculateBounds() {
		...
		textArea.setBounds(120, TABLE_HEIGHT, width_Table - 120,
				TEXTAREA_HEIGHT);
