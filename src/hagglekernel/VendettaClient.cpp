...
bool VendettaClient::determine_name(struct sockaddr &sa)
{
	...
	IPv4Address ipAddr((struct sockaddr_in&)sa);
	...
	if (iface) {
		const EthernetAddress *addr = iface->getAddress<EthernetAddress>();

		if (addr) {
			bool success = false;
			string new_name;

			new_name = do_name_lookup(addr->getStr(), success);
...
bool VendettaClient::determine_ip(SOCKET sock)
{
	...
	if (ret == -1) {
		HAGGLE_ERR("getsockname() error\n");
		return false;
	}

	memcpy(&our_ip_addr, &inaddr.sin_addr, sizeof(our_ip_addr));
	
	if (!determine_name((struct sockaddr&)inaddr)) {
		HAGGLE_ERR("Could not determine name from address\n");
		return false;
	}
