...
void ApplicationManager::onRetrieveAppNodes(Event *e)
{
	...
	*/
	...
	al = kernel->getThisNode()->getAttributes()->copy();
...
void ApplicationManager::onReceiveFromApplication(Event *e)
{
	...
	if (!appNode) {
		HAGGLE_DBG("No application node in node store matching id\n");
	}
	...
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_DEREGISTRATION_NOTICE_VALUE)) {
		HAGGLE_DBG("Application %s wants to deregister\n", appNode->getName().c_str());
		deRegisterApplication(appNode);
	}
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_REGISTER_EVENT_INTEREST_VALUE)) {
	...
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_SHUTDOWN_VALUE)) {
		if (appNode) {
			HAGGLE_DBG("Application %s wants to shutdown\n", appNode->getName().c_str());
                        kernel->shutdown();
		}
	}
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_ADD_INTEREST_VALUE)) {
	...
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_GET_INTERESTS_VALUE)) {
	...
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_GET_DATAOBJECTS_VALUE)) {
		// Clear the bloomfilter:
		appNode->getBloomfilter()->reset();
		// And do a filter matching for this node:
		updateApplicationInterests(appNode);
	}
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_DELETE_DATAOBJECT_VALUE)) {
	...
	if (appNode && dObj->getAttribute(HAGGLE_ATTR_CONTROL_NAME, HAGGLE_ATTR_REMOVE_INTEREST_VALUE)) {
		const Attributes *attrs = dObj->getAttributes();
		long numattrs = 0;
